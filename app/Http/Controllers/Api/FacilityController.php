...
class FacilityController extends Controller
{
    ...
    public function adminBook(Request $request, Facility $facility)
    {
        // 1. Strict Facility Status Guard
        if ($facility->status !== 'Active') {
            throw ValidationException::withMessages([
        ...
        $youthUser = User::findOrFail($validated['user_id']);

        // 4. Active Booking Limit per User (Max 3 active/pending bookings)
        $activeBookingsCount = $youthUser->bookingRequests()
            ->whereIn('status', ['Pending', 'Approved'])
            ->where('date', '>=', $todayDate)
            ->count();
        ...
        if ($activeBookingsCount >= 3) {
            throw ValidationException::withMessages([
                'date' => ['This user has reached the maximum limit of 3 active or pending booking requests.'],
            ]);
        }
        ...
        // Notify the youth user of immediate booking confirmation
        $youthUser->notify(new BookingConfirmedNotification($booking));

        return response()->json([
            'message' => 'Facility successfully booked and confirmed on behalf of the user.',
            'data' => $booking,
        ], 201);
    }
